...
- TTS Engine: Converts textual responses into natural voice replies (e.g., ElevenLabs, Google TTS)
- Wake Word / Toggle System: Activates listening mode and toggles always-on functionality via specific voice commands

---

### 🧠 Prioris Core AI (Orchestration Engine)

The brain of Prioris, coordinating tasks, managing context, and driving personality.
...
- Agent Orchestrator:
  - Runs background and concurrent agents
...
- Hyperbolic: Primary gateway for smart model routing (short/long context, code vs. natural language)
  - Fast queries → Claude, Sonar
  - Complex tasks → GPT-4, Claude 2.1, Haiku
  - Code generation → Claude 2.1, GPT-4
- Local Model Fallback (e.g., Ollama): Ensures Prioris can function offline or in privacy-concerned contexts
...
- Automation Bridges: IFTTT, Zapier, CLI scripting, Home Assistant
...
- Token Vault: Secure storage of API credentials (e.g., HashiCorp Vault, Keyring)
...
Defines where Prioris lives and how you interact with it.
- Hybrid Hosting Model: Self-hosted MCP/memory + optionally managed LLMs (or fully local/offline mode)
- Frontend Interface (optional): Web dashboard (React + Tailwind) or desktop/native GUI via Tauri
