...
# Prioris V0 Step 1: Developer & Testing Rules

This page contains all the Cursor rules you need to implement the Developer & Testing Tools for Prioris V0 as outlined in the [Step 1: Developer and Testing](/v0/developer-and-testing) guide.
...
touch .cursor/rules/prioris-v0-step1-setup.cursorignore
...
This rule helps you with the initial project setup for the Prioris V0 CLI tool.

<details>
<summary>**Click to see the complete Project Setup rule**</summary>

```
# Prioris V0 Project Setup Rule
...
# Rule content below
When helping with the Prioris V0 Step 1 project setup:
...
```json
{
  "name": "prioris-cli",
  "version": "0.0.1",
  "description": "Prioris CLI - Personal Assistant Prototype",
  "main": "src/index.js",
  "scripts": {
    "start": "node src/index.js"
  },
  "bin": {
    "prioris": "./src/index.js"
  },
...
This rule helps you build the CLI interface for the Prioris V0 prototype using Commander.js.

<details>
<summary>**Click to see the complete CLI Development rule**</summary>

```
# Prioris V0 CLI Development Rule
...
@file:.cursor/rules/prioris-v0-step1-setup.cursorignore

# Rule content below
When helping with the Prioris V0 CLI implementation:

1. CLI Structure Guidelines:
   - Use Commander.js for command line parsing and execution
   - Implement the following commands:
     - ask: For asking Prioris a question
...
program
  .command('ask <query>')
  .description('Ask Prioris a question')
  .action(async (query) => {
    try {
      ...
      const response = await openai.createChatCompletion({
        model: 'gpt-4',
        messages: [
          { role: 'system', content: 'You are Prioris, a helpful personal assistant.' },
          { role: 'user', content: query }
        ],
      });
      
      const answer = response.data.choices[0].message.content;
      console.log(`Prioris: ${answer}`);
  ...
  .action(async (options) => {
    try {
      ...
      interactions.forEach((item, i) => {
        ...
        console.log(`Prioris: ${item.response}`);
...
This rule helps you implement the SQLite-based storage system for the Prioris V0 prototype.

<details>
<summary>**Click to see the complete Storage Implementation rule**</summary>

```
# Prioris V0 Storage Implementation Rule
...
# Rule content below
When helping with the Prioris V0 storage implementation:

1. SQLite Setup:
   - Use sqlite3 package for database operations
   - Create a database in a user-specific location (~/.prioris/ folder)
...
const dataDir = path.join(process.env.HOME || process.env.USERPROFILE, '.prioris');
if (!fs.existsSync(dataDir)) {
  fs.mkdirSync(dataDir);
}

const dbPath = path.join(dataDir, 'prioris.db');
let db;
...
This rule helps you implement API integrations for the Prioris V0 prototype, focusing on OpenAI and Google Cloud Text-to-Speech.

<details>
<summary>**Click to see the complete API Integration rule**</summary>

```
# Prioris V0 API Integration Rule
...
# Rule content below
When helping with the Prioris V0 API integrations:
...
1. OpenAI Integration:
   ...
   - Create a consistent system prompt for Prioris
...
// Default system prompt for Prioris
const DEFAULT_SYSTEM_PROMPT = 'You are Prioris, a helpful personal assistant. Provide concise, accurate, and helpful responses.';
...
This rule helps you test and debug the Prioris V0 prototype to ensure it works correctly before moving on to later development stages.

<details>
<summary>**Click to see the complete Testing and Debugging rule**</summary>

```
# Prioris V0 Testing and Debugging Rule
...
# Rule content below
When helping with testing and debugging the Prioris V0 CLI:
...
const tests = [
  {
    name: 'Help command',
    cmd: 'prioris --help',
    expectedInOutput: 'Usage: prioris'
  },
  {
    name: 'Ask command',
    cmd: 'prioris ask "What is the capital of France?"',
    expectedInOutput: 'Paris'
  },
  {
    name: 'Remember command',
    cmd: 'prioris remember',
    expectedInOutput: 'Interaction'
  },
  {
    name: 'Speak command',
    cmd: 'prioris speak "Hello world" -o test-output.mp3',
...
// Run tests
console.log('Running Prioris CLI tests...\n');
...
2. **Create a file**: In your project, create a file at <code>.cursor/rules/prioris-v0-step1-[component].cursorignore</code>
